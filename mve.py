...
def initIndex():
    ...
    # create index
    es.indices.create(index="mve", ignore=400, body=settings)
...
#
def getGeoIPES(ip, es):


    query = '{"query":{"bool":{"must":[{"query_string":{"default_field":"ip","query":"' + ip + '"}}],"must_not":[],"should":[]}},"from":0,"size":10,"sort":[],"aggs":{}}'

    res = es.search(index="mve", doc_type="IP", body=query)

    for hit in res['hits']['hits']:

        country = "%(country)s " % hit["_source"]
        lat = "%(latitude)s " % hit["_source"]
        long = "%(longitude)s " % hit["_source"]
        countryName = "%(countryname)s " % hit["_source"]
        asn = "%(asn)s " % hit["_source"]
...
# store ip
#
def putIP(ip, country, countryname, asn, lat, long):
    ...
    try:
        ...
        res = es.index(index="mve", doc_type='IP', id=m.hexdigest(), body=vuln)
        return 0
...
# Steps
# 1. Fill Index
# 2. Speed check with MaxMind Local
# 3. Speed check with Elasticsearch
#
...
for counter1 in range(100, 101):

    for counter2 in range(1, 255):

        for counter3 in range(1, 255):

            ip = "193." + str(counter1) + "." + str(counter2) + "." + str(counter3)

            (lat, long, country, asn, countryName) = getGeoIP(ip)

            putIP(ip, country, countryName, asn, lat, long)
...
for counter1 in range(100, 101):
    ...
    for counter2 in range(1, 255):
        ...
        for counter3 in range(1, 255):
            ...
            (lat, long, country, asn, countryName) = getGeoIP(ip)
            numberIPs = numberIPs + 1

stop1 = datetime.now()

print("Elapsed time for MaxMind: " + str(stop1-start1))
...
for counter1 in range(100, 101):
    ...
    for counter2 in range(1, 255):
        ...
        for counter3 in range(1, 255):
            ...
            (lat, long, country, asn, countryName) = getGeoIPES(ip, es)

stop1 = datetime.now()

print("Elapsed time for ES: " + str(stop1-start1))

print("Total number of IPs: " + str(numberIPs))